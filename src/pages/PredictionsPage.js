...
const PredictionsPage = () => {
  ...
  const filterAndSortPredictions = () => {
    ...
    if (cmdRange.start && cmdRange.end) {
      ...
      filtered = filtered.filter(group => {
        const cmdName = group.cmdName || group.predictions?.[0]?.cmdName;
        if (!cmdName) return false;
        const cmdNum = parseInt(cmdName.replace(/\D/g, ''));
    ...
    if (dateRange.start || dateRange.end) {
      filtered = filtered.filter(group => {
        const predTime = group.predictions?.[0]?.predictionTime;
        if (!predTime) return false;
        const date = predTime.toDate ? predTime.toDate() : new Date(predTime);
      ...
      render: (_, row) => (
        <Box>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            {row.fixtureId && (
              <Chip
                label={String(row.fixtureId).replace('FIX_', 'MATCH_').substring(0, 10)}
                size="small"
                sx={{
                  backgroundColor: '#FFE5E5',
                  color: colors.brandRed,
