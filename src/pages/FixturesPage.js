...
const FixturesPage = () => {
  ...
  const [cmdRange, setCmdRange] = useState({ start: '', end: '' });
  ...
  useEffect(() => {
    const filterAndSortFixtures = () => {
      ...
      // CMd Range Filter
      if (cmdRange.start && cmdRange.end) {
        const startNum = parseInt(cmdRange.start.replace(/\D/g, ''));
        const endNum = parseInt(cmdRange.end.replace(/\D/g, ''));
        filtered = filtered.filter(fixture => {
          if (!fixture.cmdName) return false;
          const cmdNum = parseInt(fixture.cmdName.replace(/\D/g, ''));
          return cmdNum >= Math.min(startNum, endNum) && cmdNum <= Math.max(startNum, endNum);
        });
      }
      ...
      if (dateRange.start || dateRange.end) {
        filtered = filtered.filter(fixture => {
          ...
          if (dateRange.start && kickoff < dateRange.start) return false;
          if (dateRange.end && kickoff > dateRange.end) return false;
          return true;
        });
      }
      ...
      render: (_, row) => (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="caption" sx={{ color: colors.divider }}>•</Typography>
            <Typography variant="caption" sx={{ fontWeight: 600, color: colors.brandRed }}>
              {row.id?.substring(0, 10) || row.matchId || 'MATCH_???'}
            </Typography>
          </Box>
          <Chip
            label={row.matchId || 'N/A'}
            sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
          ...
          <Button
            ...
            sx={{
              ...
              fontSize: 13,
              borderRadius: '8px',
              '&:hover': { backgroundColor: `${colors.divider}1A` }
            }}
          >
            {isHistoryExpanded ? 'Hide History Filters' : 'Advanced History & Ranges'}
          </Button>
        </Box>

        {isHistoryExpanded && (
          <Box sx={{ mt: 3, pt: 3, borderTop: `1px dashed ${colors.divider}` }}>
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1.5, color: colors.brandBlack }}>
                  View CMd Range
                </Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <FormControl size="small" sx={{ flex: 1 }}>
                    <InputLabel>From CMd</InputLabel>
                    <Select
                      value={cmdRange.start}
                      onChange={(e) => {
                        setCmdRange({ ...cmdRange, start: e.target.value });
                        if (e.target.value) setSelectedCmd('all');
                      }}
                      label="From CMd"
                      sx={{ borderRadius: '10px' }}
                    >
                      <MenuItem value=""><em>None</em></MenuItem>
                      {cmds.map(cmd => <MenuItem key={cmd.id} value={cmd.name}>{cmd.name}</MenuItem>)}
                    </Select>
                  </FormControl>
                  <Typography variant="body2" sx={{ color: colors.textSecondary, fontWeight: 700 }}>→</Typography>
                  <FormControl size="small" sx={{ flex: 1 }}>
                    <InputLabel>To CMd</InputLabel>
                    <Select
                      value={cmdRange.end}
                      onChange={(e) => {
                        setCmdRange({ ...cmdRange, end: e.target.value });
                        if (e.target.value) setSelectedCmd('all');
                      }}
                      label="To CMd"
                      sx={{ borderRadius: '10px' }}
                  ...
                  {(cmdRange.start || cmdRange.end) && (
                    <IconButton size="small" onClick={() => setCmdRange({ start: '', end: '' })} sx={{ color: colors.brandRed }}>
              ...
              <Grid item xs={12} md={6}>
                  ...
                  {[
                    ...
                    <Chip
                      ...
                      sx={{
                        ...
                        fontWeight: 600,
                        '&:hover': { backgroundColor: `${colors.brandRed}0D`, borderColor: colors.brandRed }
                      }}
                    />
                  ))}
                ...
                {(dateRange.start || dateRange.end) && (
                  <Typography variant="caption" sx={{ display: 'block', mt: 1, color: colors.success, fontWeight: 700 }}>
                    Active Date Filter: {dateRange.start ? format(dateRange.start, 'MMM dd') : 'Start'} to {dateRange.end ? format(dateRange.end, 'MMM dd') : 'End'}
                  </Typography>
                )}
              </Grid>
            </Grid>
