...
const LeaguesPage = () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const formattedLeagues = result.data.leagues?.map(league => ({
          ...
          isActive: league.status?.toLowerCase() === 'active',
  ...
  const toggleLeagueStatus = async (league) => {
    ...
    try {
      ...
      if (newIsActive) {
        // Activating
        // Fetch full league details first to ensure we have all fields for PUT request
        try {
          const leagueDetails = await getLeague(league.id);

          if (leagueDetails.success && leagueDetails.data && leagueDetails.data.league) {
            const currentData = leagueDetails.data.league;

            // Construct full payload for PUT request
            const updatePayload = {
              league_name: currentData.league_name || currentData.name,
              leagueType: currentData.leagueType || currentData.type,
              logo: currentData.logo,
              status: 'Active',
              priority: currentData.priority,
              country: currentData.country,
              league_code: currentData.league_code
            };

            result = await updateLeague(league.id, updatePayload);
          } else {
            // Fallback if fetch fails, though unlikely to succeed if PUT requires all fields
            console.warn('Could not fetch full league details, attempting partial update');
          ...
          console.error('Error preparing activation:', fetchErr);
          result = { success: false, error: 'Failed to prepare league activation' };
        }
      } else {
        // Inactivating - use deactivateLeague (soft delete)
        result = await deactivateLeague(league.id);
      }

      console.log('Toggle league status API response:', result);
      ...
      if (!result.success) {
        ...
        console.log('League status updated successfully. Refreshing list...');
        // Wait a bit to ensure backend processes the change before reloading
        // Sometimes slight delay helps with consistency
        setTimeout(() => loadLeagues(), 100);
      }
    } catch (error) {
